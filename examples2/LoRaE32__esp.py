...
#             return 'NOK recvMessage'
    ...
    def sendCommand(self, command):
        ...
        if HexCmd in [0xC0, 0xC2]:        # set config to device
            ...
            HexCmd = [HexCmd]*3
        if self.debug:
            print('2 HexCmd', HexCmd, bytes(HexCmd))
        if self.config['baudrate'] != 9600:
            #self.serdev.baudrate = 9600;
            self.serdev.init(baudrate = 9600) #, timeout=100)
            print(1, self.serdev)
        ...
        if self.config['baudrate'] != 9600:
            ...
            self.serdev.init(baudrate = self.config['baudrate']) #, timeout=100)
            print(2, self.serdev)
        return result
